...
tiles = suppressMessages(getTiles(gridFile, args['region'], args['from'], args['to'], compositeBands, args['user'], args['pswd'], tilesDir)) %>%
  mapTilesPeriods(args['period'], args['from']) %>%
  mutate(whichFile = getTilePath(tilesDir, .data$tile, .data$period, args['whichBand'])) %>%
...
composites = foreach(tls = assignToCores(tiles, nCores, chunksPerCore), .combine = bind_rows) %dopar% {
  ...
  suppressMessages(prepareComposites(tls, tilesDir, tmpDir, compositeSkipExisting))
}

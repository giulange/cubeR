...
#' @param user s2.boku.eodc.eu service user password
...
getTiles = function(gridFile, roi, dateFrom, dateTo, bands, user, pswd, tilesDir) {
  ...
  tiles = suppressMessages(getImages(roi, dateFrom, dateTo, '', projection, 'SCL')) %>%
    ...
    dplyr::select(date, tile) %>%
    dplyr::distinct() %>%
    dplyr::mutate(x = 1) %>%
    dplyr::inner_join(dplyr::tibble(x = 1, band = bands)) %>%
    dplyr::select(-x) %>%
    dplyr::mutate(tileFile = getTilePath(tilesDir, tile, date, band)) %>%
    dplyr::arrange(date, band, tile)
  return(tiles)
}

...
#' @import dplyr
#' @export
retile = function(images, targetDir, gridFile, tmpDir) {
  options(scipen = 100)
  ...
  imgs = images %>%
    ...
    dplyr::mutate(
      command = paste('gdalwarp -overwrite -tr 10.0 10.0 -srcnodata', nodata, '-t_srs', paste0('"', projection, '"'), paste0('"', path.expand(file), '"'), paste0('"', equi7File, '"'))
    )
  imgs = imgs %>%

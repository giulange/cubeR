...
#' @param period 'N days' or 'N months' or 'N years'
#' @param aggBand name of a band to be aggregated
#' @param maskBand name of a band to be used as a cloud mask
#' @return data frame describing computed aggregated images
#' @import dplyr
#' @export
# prepareAggregates = function(tiles, targetDir, tmpDir, period, aggBand, maskBand) {
#   unit = sub('^.* ', '', period)
...
#     ) %>%
#     dplyr::rename_(period = unit) %>%
#     dplyr::mutate(
#       period = as.integer((period - 1) / len),
...
#
#   bandMax = nodata %>%
#     dplyr::filter(band == aggBand) %>%
#     dplyr::group_by(period, tile) %>%
#     dplyr::arrange(period, tile) %>%
#     dplyr::mutate(
...
#
#   agg = nodata %>%
#     dplyr::group_by(period, tile, band) %>%
#     dplyr::arrange(period, tile, band) %>%
#     dplyr::mutate(
...
#       )
#     ) %>%
#     dplyr::inner_join(bandMax) %>%
#     dplyr::mutate(
#       aggFile = paste0(targetDir, '/', period, '_', tile, '_', band, '.tif'),
#     ) %>%
#     dplyr::group_by(period, tile, band) %>%
#     dplyr::mutate(
